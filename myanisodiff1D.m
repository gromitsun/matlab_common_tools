function sig = myanisodiff1D(sig, num_iter, delta_t, kappa, option, verbose)
...
for t = 1:num_iter
    ...
    nabla = imfilter(sig,hW,'conv');
    % Diffusion function & Discrete PDE solution.
    if option == 1
        diff_sig = (1/(dx^2)) * exp(-(nabla/kappa).^2) .* nabla;
    elseif option == 2
        diff_sig = (1/(dx^2)) ./(1 + (nabla/kappa).^2) .* nabla;
    end
    
    % E
    % Finite differences. [imfilter(.,.,'conv') can be replaced by conv2(.,.,'same')]
    nabla = imfilter(sig,hE,'conv');
    % Diffusion function & Discrete PDE solution.
    if option == 1
        diff_sig = diff_sig + (1/(dx^2)) * exp(-(nabla/kappa).^2) .* nabla;
    elseif option == 2
        diff_sig = diff_sig + (1/(dx^2)) ./(1 + (nabla/kappa).^2) .* nabla;
    end
    
    % Step forward PDE
    sig = sig + delta_t * diff_sig;
