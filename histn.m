function [nn,ctrs] = histn(varargin)
...
dflts =  { [],     [],       []};
[nbins,ctrs,edges,~,~] = internal.stats.parseArgs(pnames,dflts,args{:});
...
if ~isempty(nbins)
    ...
    % to be determined.
    histBehavior = true;
    if ~(isnumeric(nbins) && numel(nbins)==ncols)
...
elseif ~isempty(ctrs)
    ...
    histBehavior = true;
    if ~(iscell(ctrs) && numel(ctrs)==ncols && isnumeric(ctrs{1}) && isnumeric(ctrs{2}))
